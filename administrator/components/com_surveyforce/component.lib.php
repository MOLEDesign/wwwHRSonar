...
if (!class_exists('JTableComponent')) {
	JLoader::load('JTableComponent');
}
...
if (!class_exists('JTableSection')) {
	JLoader::load('JTableSection');
}
...
class mosToolBar {
	...
	* @param string An override for the alt text
	*/
	function unarchiveList( $task='unarchive', $alt='COM_SF_UNARCHIVED' ) {
		$alt= JText::_( $alt );
