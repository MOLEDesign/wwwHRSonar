...
***********************************************************************************************************************/
	function SF_MainLayout() {
		...
		$return_str = <<<EOF_RES
			...
			<link rel="stylesheet" type="text/css" href="{$mosConfig_live_site}/media/surveyforce/surveyforce_new/surveyforce.css"/>
			
			<div class="componentheading" id="surveyforce">{SURVEY_NAME}</div>
...
**************************************************************************************************************************************/
				...
				for (i = 0; i < mcount; i++) {
					...
					for (j = 0; j < scount; j++) {
						...
						return_str = return_str + 
									'<td class="ls_answer_cell" onclick="javascript: changeImage(\'quest_radio_'+questions[n].cur_quest_id+'_'+SF_getElement(data, 'mfield_id', i)+'\', '+j+', '+scount+'); check_answer('+n+');">' +
	...
	function SF_GetStartButton() {
		global $sf_lang;
		return '<div class="start_button"><a class="start_link" onfocus="javascript: this.blur();" href="javascript: void(0);" onclick="javascript: sf_StartSurveyOn()">'.$sf_lang['SURVEY_START'].'</a></div>';
	}
}//class..
}//if 
