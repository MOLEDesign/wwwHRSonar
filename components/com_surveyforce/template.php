...
***********************************************************************************************************************/
	function SF_MainLayout() {
		...
		$return_str = <<<EOF_RES
			...
			<link rel="stylesheet" type="text/css" href="{$mosConfig_live_site}/components/com_surveyforce/surveyforce.css"/>
...
**************************************************************************************************************************************/
		...
		<script language="javascript" type="text/javascript"><!--//--><![CDATA[//><!--
			function getLikertScale(n) {
				...
				for (i = 0; i < mcount; i++) {
					...
					return_str = return_str + '<tr class="sectiontableentry'+k+'"><td class="ls_quest_field">' + SF_getElement(data, 'mfield_text', i) + '</td>';
					...
					if (quest_style == 1) {
						...
						return_str = return_str + 
									...
									'<td class="po_answer_cell">'+
										'<input onchange="javascript: check_answer('+n+');" class="po_radio" type="radio" name="quest_radio'+questions[n].cur_quest_id+'" id="quest_radio'+questions[n].cur_quest_id+i+'" value="'+SF_getElement(data, 'mfield_id', i)+'" '+selected+'>'+
									'</td>'+
	...
	function showCategoryView($cat, $rows) {
		global $sf_lang, $mosConfig_live_site;
		?>
		<link rel="stylesheet" type="text/css" href="<?php echo $mosConfig_live_site;?>/components/com_surveyforce/surveyforce.css"/>
		<div class="contentpane">
			<div class="componentheading">
				<?php echo $sf_lang['SURVEY_CATEGORY']."&nbsp;".$cat->sf_catname;?>
			</div>
			<div style="padding-left:10px;">
				<?php echo $cat->sf_catdescr;?>
			</div>
			<br />
		
			<table width="100%" cellpadding="0" cellspacing="0" border="0" id="surv_list" class="contentpane">
			<tr><td class="sectiontableheader">
				<strong><?php echo $sf_lang['SURVEY_SURVEY'];?></strong>
			</td></tr>
			<?php
			$k = 2;
			foreach($rows as $row) { ?>
				<tr  class="<?php echo "sectiontableentry$k"; ?>"><td style="padding-left:10px;"><strong><a href="<?php echo SFRoute_fe("index.php?option=com_surveyforce&survey=".$row->id);?>"><?php echo $row->sf_name;?></a></strong></td></tr>
				<tr  class="<?php echo "sectiontableentry$k"; ?>"><td style="padding-left:15px;"><?php echo $row->surv_short_descr;?></td></tr>
				<?php
				$k = 3 - $k;	
			}?>
			</table>	
