...
class survey_force_html 
{
	function processTemplate($template, $sf_config, $survey, $show_description) {
		...
		if ($survey->sf_image) {
			$template = preg_replace('/\{BACKGROUND_IMAGE\}/', "background: url(images/surveyforce/".$survey->sf_image.") no-repeat;", $template, 1);	
		} 
	...
	function get_progress_bar($sf_config) {
		...
		$str = '<div id="progress" style="display:none">
					...
					<div id="progress_bar_txt"  style="float: center; position:relative; top:-16px;color:#'.$sf_config->get('color_text').'; ">'.$sf_lang['SF_PROGRESS'].' 0%</div>		
				</div>';
	...
	function Survey_blocked($sf_config)
	{
		global $mosConfig_live_site, $sf_lang;
	?>		
	<script language="JavaScript" src="<?php echo $mosConfig_live_site?>/components/com_surveyforce/bits_mycommon.js" type="text/javascript"></script>
	...
	<!--//--><![CDATA[//><!--
		var mes_not_avail = '<?php echo $sf_lang['SURVEY_NOT_AVAIL']?>';
	...
	<div class="componentheading"><?php echo $sf_lang['COMPONENT_HEADER']?></div>
	...
	{
		global $mosConfig_live_site, $sf_lang, $my;
	...
	if (questions[n].cur_quest_type == 3) {
		...
		if (sf_num_options > 0) {
			...
			if (checked_item >= sf_num_options) {				
				ShowMessage('error_messagebox', 1, '<?php echo $sf_lang['CANNOT_CHECK_ANYMORE'];?>');
				elem.checked = false;
				return false;
			}
		}	
...
// *** DRAG'and'DROP CODE *** //
var color_cont = '#<?php echo $sf_config->get('color_cont')?>';
...
// *** MESSAGES *** 
var mes_enter_some_words = '<?php echo $sf_lang['COMPLETE_SHORT_ANSWER']?>';
var mes_select_one_radio = '<?php echo $sf_lang['COMPLETE_PICK_ONE']?>';
var mes_select_some_checks = '<?php echo $sf_lang['COMPLETE_PICK_MANY']?>';
var mes_select_your_choice = '<?php echo $sf_lang['COMPLETE_DROP_DOWN']?>';
var mes_select_your_rank = '<?php echo $sf_lang["COMPLETE_RANK"]?>'; 
var mes_complete_this_part = '<?php echo $sf_lang['COMPLETE_LIKERT']?>';
var mes_complete_this_part_drag = '<?php echo $sf_lang['COMPLETE_DRAG_AND_DROP']?>';
var mes_complete_imp_scale = '<?php echo $sf_lang['COMPLETE_IMPORTANT_SCALE']?>';

var mes_loading = '<?php echo $sf_lang['SURVEY_LOAD_DATA']?>';
var mes_failed = '<?php echo $sf_lang['SURVEY_FAILED']?>';
var mes_invite_complete = '<?php echo $sf_lang['SURVEY_INVITED_COMPLETE']?>';
var mes_reg_complete = '<?php echo $sf_lang['SURVEY_REG_COMPLETE']?>';
var mes_pub_complete = '<?php echo $sf_lang['SURVEY_PUB_COMPLETE']?>';
var mes_please_wait = '<?php echo $sf_lang['SURVEY_PLEASE_WAIT']?>';

var mes_session_timed_out = '<?php echo $sf_lang['SESSION_TIMED_OUT']?>';
...
var url_prefix = 'index2.php?option=com_surveyforce&task=ajax_action<?php echo ($preview? '&preview=1':'')?>';
...
function sf_AnalizeRequest(http_request) {
	if (http_request.readyState == 4) {
		if ((http_request.status == 200)) {
			...
			ShowMessage('error_messagebox', 1, '<?php echo $sf_lang['SURVEY_FAILED_REQUEST']?> ('+http_request.status+')');
		}
	}
}
function sf_AddEvent(obj, evType, fn){
...
function sf_StartSurvey() {
	...
	if (!confirm("<?php echo $sf_lang['SF_ALREADY_COMPLETED_REG']; ?>")) {
		ShowMessage('error_messagebox', 1, '');
		window.history.go(-1);
		return;		
	}			
...
function sf_SurveyNextData(n) { //proveriaem vse li otmecheno i vozvraschaem stroku s otvetami
	...
	switch (questions[n].cur_quest_type) {
		...
		break;
		default:
			ShowMessage('error_messagebox', 1, '<?php echo $sf_lang['SURVEY_UNKNOWN_ERROR']?>');
			ShowMessage('error_messagebox2', 1, '<?php echo $sf_lang['SURVEY_UNKNOWN_ERROR']?>');
			return false;
		break;
	}
		...
		case 'start':
			for(i=0; i<next_count; i++) {
				try {
					...
					sf_getObj('next_'+i).innerHTML = getButton('next', '<?php echo $sf_lang['SURVEY_NEXT_QUEST']?>', 'sf_SurveyNextOn()');
					} catch(e) {} 
			}
		break;		
		case 'next':
			...
			for(i=0; i<prev_count; i++) {
				try {
					...
					sf_getObj('prev_'+i).innerHTML = getButton('prev', '<?php echo $sf_lang['SURVEY_PREV_QUEST']?>', 'sf_SurveyPrevOn()');
				} catch(e) {} 
			}
		...
		case 'start_last_question':	
			...
			for(i=0; i<finish_count; i++) {
				try {
					...
					sf_getObj('finish_'+i).innerHTML = getButton('finish', '<?php echo $sf_lang['SURVEY_SUBMIT_SURVEY']?>', 'sf_SurveyNextOn()');
				} catch(e) {} 
			}
